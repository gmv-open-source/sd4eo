# File Name:     synthpatch_assembly.py
...
def selectBestPatch(spec_str, base_path, city_id, col, row, target_osm_res, t_ovelap_margin, rgb_canvas, canvas_pixel_y, canvas_pixel_x):
    """
    ...
    for filename in list_filenames:
        ...
        if current_measured_distance < min_error:
            ...
            selected_patch = scaled_patch
            debug_selected_filename = filename
    print(f'selected: {debug_selected_filename}')
...
def selectRandomPatch(spec_str, base_path, city_id, col, row, target_osm_res, forced_candidate=-1):
    ...
    if forced_candidate >= 0 and forced_candidate < count:
        ...
        candidate_filename = list_filenames[random.randint(0, count)]
...
if __name__ == '__main__':
    # SELECTED CITY + main parameters:
    city_id = available_cities_IDs[3] 
    IR_flag = True
    mode = 'cut' # 'best' # 'random'
    ...
    original_output_filename = f'./assembled_{city_id}_{spec_mode}.png'
    S2scaled_output_filename = f'./assembled_S2_{city_id}_{spec_mode}.png'
    ...
    for row in range(max_row-1,-1,-1):
        ...
        for col in range(max_col):    
            ...
            if mode == 'random' or (row == max_row-1 and col == 0):
                ...
                selected_patch = selectBestPatch(spec_mode, input_base_path, city_id, col, row, target_osm_res, t_ovelap_margin, rgb_canvas, canvas_pixel_y_ini, canvas_pixel_x_ini)
                if mode == "cut":
    ...
    io.show()
    save_rgb_image(rgb_canvas, original_output_filename)
