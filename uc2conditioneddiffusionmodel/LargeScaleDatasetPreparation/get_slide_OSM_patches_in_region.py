# File Name:     get_slide_OSM_patches_in_region.py
...
import numpy as np
import time
from common_tile_utils import available_Tiles_IDs, available_cities_IDs, provide_inner_tile_coords
...
class SentinelTile:
    ...
    def get_num_overlapped_patches_inside(self, patch_extent, patch_extent_crs, overlap_factor):
        # overlap_factor is always positive and < 1.0
        if self.tile_crs != patch_extent_crs:
            conv_patch_extent = transform_qgsrectangle(patch_extent, patch_extent_crs, self.tile_crs)
        else:
            conv_patch_extent = patch_extent

        num_width = int((self.valid_extent.width() - conv_patch_extent.width()) / (conv_patch_extent.width()*overlap_factor)) + 1
        num_height = int((self.valid_extent.height() - conv_patch_extent.height()) / (conv_patch_extent.height()*overlap_factor)) + 1
        print(f'num patches inside Tile {self.id} : # width: {num_width}  /  # height: {num_height}')
        return num_width, num_height
    ...
    def OSM_scan_area_inside_tile(self, example_patch_extent, patch_extent_crs, output_folder):
        ...
        # It works ONLY in contigous mode
    ...
    def OSM_overlapped_scan_area_inside_tile(self, example_patch_extent, patch_extent_crs, output_folder, overlap_factor=0.2):
        # We sample all the tile surface and dump OpenStreetMap patches as small PNG files
        # It works ONLY in overlapped mode
        num_width, num_height = self.get_num_patches_inside(example_patch_extent, patch_extent_crs)
        if self.tile_crs != patch_extent_crs:
            conv_patch_extent = transform_qgsrectangle(example_patch_extent, patch_extent_crs, self.tile_crs)
        else:
        ...
        num_processed_patches = 0
        num_total_patches = num_width * num_height
        fixed_width_step = conv_patch_extent.width() * (1.0-overlap_factor)
        fixed_height_step = conv_patch_extent.height() * (1.0-overlap_factor)
        time.sleep(0.4)
        extent_reg = np.zeros((num_width, num_height,4))
        for x_i in range(num_width):
            for y_i in range(num_height):
                min_x_i_patch = min_x_1st_patch + fixed_width_step * x_i
                min_y_i_patch = min_y_1st_patch + fixed_height_step * y_i
                max_x_i_patch = max_x_1st_patch + fixed_width_step * x_i
                max_y_i_patch = max_y_1st_patch + fixed_height_step * y_i
                patch_i_extent = QgsRectangle(min_x_i_patch, min_y_i_patch, max_x_i_patch, max_y_i_patch)
                print(f' OSM patch x: {x_i}  y: {y_i}   -->   ({num_processed_patches} / {num_total_patches}) ')
                output_file_png = output_folder + f'patch_{self.id}_{x_i:03}_{y_i:03}_OSM.png'
                export_osm_image_by_extent(patch_i_extent, output_file_png)
...
# available_cities_IDs = ['Paris', 'Madrid', 'Toulouse', 'Poitiers',]

if __name__ == '__console__': # or __name__ == '__main__'
    warmp_up_flag = True # False # True
    default_output_file_png = "C:/DMTA/projects/SD4EO/map-epiphany/output_file.png"
    # madrid_tile_EPSG32630 = QgsRectangle(400375, 4499727, 509067, 4391081)
    # madrid_tile_EPSG4326 = QgsRectangle(40.63831, -4.17545, 39.66683, -2.89456)
    # madrid_tile_EPSG3857 = transform_qgsrectangle(madrid_tile_EPSG32630, 'EPSG:32630', 'EPSG:3857')
    ...
    if warmp_up_flag:
        ...
        rect_32630 = transform_qgsrectangle(rect_3857, 'EPSG:3857', 'EPSG:32630')
        # estimate_num_tiles(rect_32630, madrid_tile_EPSG32630)
        for city_subtile_id in available_cities_IDs:
            print(f'Processing city : {city_subtile_id}')
            output_OSM_png_folder = f'C:/DATASETs/UC2Synthesis/OSM/{city_subtile_id}/'
            icoords_EPGS3857, icoords_EPGS4326, base_filename, orig_tile_name = provide_inner_tile_coords(city_subtile_id)

            tile_i = SentinelTile(city_subtile_id, f'C:/DATASETs/UC2Training/Sentinel2/{orig_tile_name}/', base_filename, QgsRectangle(*icoords_EPGS3857), 'EPSG:3857') #madrid_tile_EPSG32630, 'EPSG:32630')
            print('Hi!')
            tile_i.OSM_overlapped_scan_area_inside_tile(rect_3857, 'EPSG:3857', output_OSM_png_folder, overlap_factor)
